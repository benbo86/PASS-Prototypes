...
// Shared "assign employee(s) to a visit" panel — extracted from
// schedule/assign-visit-absent-employee-event-panel's own page-level
// EventPanel.jsx (the first place this UI existed) so schedule/daily-
// schedule/'s Unassigned-visit assign flow reuses the exact same
// component instead of a parallel, separately-maintained one. See both
// consuming files for how each wires it up — this file owns only the
// scrim/panel/header/tabs/body/footer chrome and its own internal state
// (date/time editing, notes, slot assignment, the holiday-conflict
// warning); everything content-specific comes in as props.
//
// New, purely additive since this was extracted (none of these change
// anything for the original absent-employee prototype unless it opts in):
// `tone: 'amber'` status badge (for a genuinely unassigned visit, vs the
// original's fixed purple "Booked" stripe), `tags`/`address`/`phone`
// (all optional, render nothing if omitted), `showRequiredWarning` (a
// red "Visit requires N employees" banner while under-staffed), and
// `requireAtLeastOneAssigned` (disables Save until at least one slot is
// filled). `onClose`/the Cancel button/scrim-click are now genuinely
// wired — the original page had no "close" destination to wire them to,
// so its own X/Cancel buttons were previously inert (Cancel's onClick
// was even a stray `setShowWarning(false)`, not a real close — fixed here
// as part of the extraction, see the CLAUDE.md entry for this round).
...
function StatusBadge({ label, tone }) {
  if (tone === 'amber') return <span className="ep-unassigned-badge">{label}</span>
  return (
...
function CustomerAvatarImg({ url, initials, bg, name }) {
  if (url) return <img src={url} alt={name} className="ep-avatar ep-avatar--customer" />
  return (
    <div className="ep-avatar ep-avatar--customer ep-avatar--initials" style={{ background: bg }}>
      {initials}
    </div>
  )
}
...
export default function EventPanel({
  ...
  customerName,
  visitLabel,
  visitKindLabel,          // optional plain-text label next to the customer name, e.g. "Care visit"
  tags,                    // optional ReactNode — rendered as its own row under the title (e.g. Area/Visit Type pills)
  initialDateISO,
  initialStart,
  initialEnd,
  plannedStart,            // optional — only shown (struck through) if both are given
  plannedEnd,
  recurrenceText,          // optional
  address,                 // optional
  phone,                   // optional
  allowNotes = true,

  // Tabs — content is only ever rendered for tabs[0] (matches the
  // original's own pre-existing behaviour: every other tab was already
  // just an inert label with no body).
  tabs = ['Assigned Today', 'Recurring Employees', 'Care Required', 'History', 'Customer Details'],

  // Assignment
  employeesRequired,
  initialSlots,            // array, length === employeesRequired, each an employee object or null
  recommendedEmployees,
  showRequiredWarning = false,
  requireAtLeastOneAssigned = false,
  onSave,                  // (finalSlots) => void — called once Save/Accept-assignment completes
}) {
  const [activeTab, setActiveTab] = useState(tabs[0])
  const [slots, setSlots] = useState(initialSlots)
  const [openMenu, setOpenMenu] = useState(null)

  const [visitDateISO, setVisitDateISO] = useState(initialDateISO)
  const [visitStart, setVisitStart] = useState(initialStart)
  const [visitEnd, setVisitEnd] = useState(initialEnd)
  ...
  const handleEditNote = () => { setDraftText(savedNote.text); setEditingNote(true) }
  const handleDeleteNote = () => { setSavedNote(null); setEditingNote(false); setDraftText('') }
  ...
  const allFilled = assignedCount === employeesRequired
  ...
  const toggleMenu = (slotIndex) => setOpenMenu(prev => prev === slotIndex ? null : slotIndex)

  const handleSave = () => {
    const hasHolidayEmployee = slots.some(s => s && s.onHoliday)
    if (hasHolidayEmployee) { setShowWarning(true); return }
    onSave?.(slots)
  }
  ...
  const holidayNames = slots.filter(s => s && s.onHoliday).map(s => s.name).join(' and ')
      ...
      <div className="ep-panel" role="dialog" aria-modal="true" aria-label="Event panel">
          ...
          <div className="ep-header__top">
            <div className="ep-header__title-row">
              <h1 className="ep-header__title">
                {visitLabel} <span className="ep-title-with">with</span> {customerName}
              </h1>
              <StatusBadge {...statusBadge} />
            </div>
            <button className="ep-close-btn" onClick={onClose} aria-label="Close panel">
              <CloseIcon />
            </button>
          </div>

          {tags && <div className="ep-tags-row">{tags}</div>}

          <div className="ep-header__details">
            <CustomerAvatarImg url={customerAvatarUrl} initials={customerInitials} bg={customerAvatarBg} name={customerName} />
            <div className="ep-header__meta">
              <div className="ep-header__meta-row">
                <div className="ep-header__name-group">
                  ...
                  <strong className="ep-header__meta-name">{customerName}</strong>
                </div>

                {visitKindLabel && <span>{visitKindLabel}</span>}

                {editingDate ? (
                  <div className="ep-inline-edit" onClick={e => e.stopPropagation()}>
                    <input
                      type="date" className="ep-edit-input" value={visitDateISO}
                      onChange={e => setVisitDateISO(e.target.value)} autoFocus
                    />
                  ...
                  <span className="ep-inline-row">
                    ...
                    <button className="ep-icon-btn" onClick={e => { e.stopPropagation(); setEditingDate(true); setEditingTime(false) }}>
                      <EditIcon />
                    </button>
                  </span>
                )}
                ...
                <div className="ep-time-col">
                  {plannedStart && plannedEnd && (
                    <span className="ep-planned">Planned time: <s>{plannedStart} - {plannedEnd}</s></span>
                  )}
                  {editingTime ? (
                    <div className="ep-inline-edit" onClick={e => e.stopPropagation()}>
                      <input type="time" className="ep-edit-input ep-edit-input--time" value={visitStart} onChange={e => setVisitStart(e.target.value)} autoFocus />
                      <span>–</span>
                      <input type="time" className="ep-edit-input ep-edit-input--time" value={visitEnd} onChange={e => setVisitEnd(e.target.value)} />
                    ...
                    <span className="ep-inline-row">
                      ...
                      <button className="ep-icon-btn" onClick={e => { e.stopPropagation(); setEditingTime(true); setEditingDate(false) }}>
                        <EditIcon />
                      </button>
                    </span>
                  )}
                ...
                {recurrenceText && (
                  <span className="ep-recurrence"><RecurIcon /> {recurrenceText} <ChevronDownIcon /></span>
                )}
              ...
              {(address || phone) && (
                <div className="ep-header__extra-row">
                  {address && <span className="ep-inline-row ep-inline-row--static"><MapPinIcon /> {address}</span>}
                  {phone && <span className="ep-inline-row ep-inline-row--static"><PhoneIcon /> {phone}</span>}
                </div>
              )}
            </div>
          </div>

          {allowNotes && (showNoteInput || editingNote || savedNote) && (
              ...
              {savedNote && !editingNote ? (
                <div className="ep-note-saved">
                  ...
                  <div className="ep-note-saved__actions">
                    <button className="ep-note-action-btn" onClick={handleEditNote} aria-label="Edit note"><EditIcon /></button>
                    <button className="ep-note-action-btn" onClick={handleDeleteNote} aria-label="Delete note"><DeleteIcon /></button>
                  </div>
                </div>
              ) : (
                <>
                  <textarea
                    className="form-textarea ep-note-textarea" placeholder="Add a visit note…"
                    value={draftText} onChange={e => setDraftText(e.target.value)} autoFocus
                  />
                  <div className="ep-note-btns">
                    <button className="round-btn tertiary-btn" onClick={() => { setShowNoteInput(false); setEditingNote(false); setDraftText('') }}>Cancel</button>
                    <button className="round-btn primary-btn" disabled={!draftText.trim()} onClick={handleSaveNote}>Save note</button>
                  </div>
                </>
              )}
            </div>
          ...
          {activeTab === tabs[0] && (
            <>
              <h2 className='ep-section-title'>
                {employeesRequired} Employee{employeesRequired > 1 ? 's' : ''} required{' '}
                <span className="ep-section-count">({assignedCount}/{employeesRequired} assigned)</span>
              </h2>
              <div className="ep-bars">
                {slots.map((employee, i) =>
                  employee
                    ? <FilledSlotBar key={i} employee={employee} slotIndex={i} menuOpen={openMenu === i} onMenuToggle={toggleMenu} onUnassign={unassignEmployee} />
                    : <UnassignedBar key={i} slotNumber={i + 1} />
                )}
              </div>

              {showRequiredWarning && !allFilled && (
                <div className="warning-banner red">
                  <WarningIcon />
                  <div><p>Visit requires {employeesRequired} employee{employeesRequired > 1 ? 's' : ''}</p></div>
                </div>
              )}
              ...
              <div className="ep-bars">
                {recommendedEmployees.map(emp => (
                  <AssignBar key={emp.id} employee={emp} onAssign={assignEmployee} disabled={allFilled || slots.some(s => s && s.id === emp.id)} />
                ))}
              </div>
            </>
          )}
        ...
        <div className="ep-footer">
          {showWarning && (
            <div className="ep-footer__warning-content">
              ...
              <div className="warning-banner orange">
                ...
                <div>
                  <h4>Warnings</h4>
                  <ul><li>{holidayNames} is on holiday on {formatDisplayDate(visitDateISO)}</li></ul>
                </div>
              </div>
            </div>
          )}
          <div className={`ep-footer__actions${showWarning ? ' ep-footer__actions--warning' : ''}`}>
            {!showWarning && (
              <button className="round-btn secondary-btn" onClick={onClose}>Cancel</button>
            )}
            <div className="ep-footer__btn-group">
              ...
              {showWarning ? (
                <button className="round-btn primary-btn" onClick={handleAssignAnyway}>Accept assignment</button>
              ) : (
                <button className="round-btn primary-btn" onClick={handleSave} disabled={requireAtLeastOneAssigned && assignedCount === 0}>Save</button>
              )}
            </div>
          </div>
        </div>
